...
import { createChapter, extractChapterText } from '../utils/api';
...
export default function CreateChapterScreen({ route, navigation }) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      Alert.alert('Thành công', t('chapterCreated'), [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);
    } catch (error) {
      Alert.alert('Lỗi', 'Không thể thêm chương mới. Bạn có chắc mình là người tạo truyện này?');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('addChapter')}</Text>
        <View style={{ width: 40 }} />
      </View>

      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.novelName}>Truyện: {novelTitle}</Text>
        ...
        >
          {loading ? (
             ...
             <Text style={styles.submitBtnText}>{t('publishChapter')}</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
    </View>
