...
import { updateNovel , getImageUrl } from '../utils/api';
...
export default function EditNovelScreen({ route, navigation }) {
  ...
  // Load draft on mount (removed for edit screen to avoid confusion)
  useEffect(() => {
    // No draft for edit
  }, []);

  // Save draft removed for edit screen
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sửa Thông Tin Truyện</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={{ flexDirection: 'row' }}>
            {['Ongoing', 'Completed', 'Hiatus'].map((s) => (
              <TouchableOpacity
                key={s}
                style={[styles.genreChip, status === s && styles.genreChipActive]}
                onPress={() => setStatus(s)}
              >
                <Text style={[styles.genreText, status === s && styles.genreTextActive]}>{s}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        >
          {loading ? (
             ...
             <Text style={styles.submitBtnText}>Cập Nhật Truyện</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
    </View>
